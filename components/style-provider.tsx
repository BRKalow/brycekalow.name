...
const StyleProvider: React.FC<{ children: ReactElement | ReactElement[] }> = ({
  ...
  const [theme, setTheme] = React.useState<Themes>(() =>
    typeof window !== "undefined" ? window.__theme ?? "initial" : "initial"
  );
  const toggleTheme = React.useCallback(
    () => setTheme((cur) => (cur === "initial" ? "dark" : "initial")),
    []
  );
  ...
  const themeContextValue = React.useMemo(
    () => ({
      theme: { ...themes["initial"], ...themes[theme] },
      name: theme,
      toggleTheme,
    }),
    [theme, toggleTheme]
  ...
  React.useEffect(() => {
    if (isMounted && !shouldTransitionTheme.current)
      shouldTransitionTheme.current = true;
  }, [isMounted]);
      ...
      <style jsx global>{`
        ...
        * {
          --gradient-background: linear-gradient(
            var(--gradient-background-rotation),
            #12c2e9,
            #c471ed,
            #f64f59
          );
