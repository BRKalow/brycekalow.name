...
export async function getPosts(): Promise<fs.Dirent[]> {
    return (await fs.promises.readdir(path.join(process.cwd(), 'content'), { withFileTypes: true }))
...
export async function getPostList(): Promise<PostWithMetadata[]> {
    ...
    return await Promise.all(posts.map(async post => {
        const contents = await fs.promises.readFile(path.join(process.cwd(), 'content', post.name), 'utf-8');
        return {
